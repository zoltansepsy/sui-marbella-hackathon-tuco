...
export function JobDetailView({ jobId, open, onClose, onApplySuccess }: JobDetailViewProps) {
  ...
  // Fetch job description from Walrus (if available)
  useEffect(() => {
    if (job && job.descriptionBlobId && open) {
      setLoadingDescription(true);
      // TODO: Integrate with WalrusService to fetch description
      // For now, use placeholder
      setTimeout(() => {
        setDescription(`Job description for: ${job.title}\n\nThis is a detailed description that would be fetched from Walrus storage.\n\nBlobId: ${job.descriptionBlobId}`);
                ...
                <Card>
                  ...
                  <CardContent>
                    <p className="text-sm text-muted-foreground">
                      Milestone details will be revealed after job assignment
                    </p>
                  </CardContent>
                </Card>
              )}
